...
const registerUser = (req, res) => {
    ...
    const registeredUser = userService.registerUser(newUser)
        .then(registeredUser => {
            ...
            else res.status(201).send({ status: 'OK', message: 'User successfully registered' })
        })
        .catch(error => {
            console.log('Error: ', error)
            res.status(500).send({ status: 'Error', message: 'Error registering the new user' })
...
const updateUser = (req, res) => {
    ...
    const updatedUser = userService.updateUser(userId, newUser)
        .then(updatedUser => {
            ...
            else res.status(200).send({ status: 'OK', message: 'User updated successfully' })
        })
        .catch(error => {
            console.log('Error: ', error)
...
const updatePassword = (req, res) => {
    ...
    const updatedUser = userService.updatePassword(body.email, body.newPassword)
        .then(updatedUser => {
            ...
            else res.status(200).send({ status: 'OK', message: 'Password updated successfully' })
        })
        .catch(error => {
            console.log('Error: ', error)
            res.status(500).send({ status: 'Error', message: 'Error updating the password' })
